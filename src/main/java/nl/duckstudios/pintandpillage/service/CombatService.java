...
public class CombatService {
    ...
    public void checkHasEnoughShipsToSendUnits(List<VillageUnit> villageUnits) {
        ...
        List<VillageUnit> meme = villageUnits.stream()
                .filter(u -> !(u.getUnit() instanceof Scout) && !(u.getUnit() instanceof ShipUnit)).toList();

        int totalUnitCapacity = meme.stream().mapToInt(u -> u.getUnit().getPopulationRequiredPerUnit() * u.getAmount()).sum();
